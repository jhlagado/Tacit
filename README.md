...
Tacit is a minimalist character-based interpreter but one which aims at fast performance, readability and ease of use. It is written for the Z80 microprocessor and is 2K.

- [What is Tacit?](#what-is-Tacit)
- [Reverse Polish Notation (RPN)](<#reverse-polish-notation-(rpn)>)
- [Numbers in Tacit](#numbers-in-Tacit)
...
- [Stack Manipulation in Tacit](#stack-maniplation-in-Tacit)
  - [Duplicate](#duplicate)
  - [Drop](#drop)
  - [Swap](#swap)
  - [Over](#over)
...
## <a name='what-is-Tacit'></a>What is Tacit?

Tacit is a bytecode interpreter - this means that all of its instructions are 1 byte long. However,
...
Here is an example of a simple Tacit program that uses RPN:
...
## <a name='numbers-in-Tacit'></a>Numbers in Tacit

Tacit on the Z80 uses 16-bit integers to represent numbers. A valid (but not very
interesting) Tacit program can be simply a sequence of numbers. Nothing will happen
to them though until the program encounters an operator.

There are two main types of numbers in Tacit: decimal numbers and hexadecimal numbers.

### <a name='decimal-numbers'></a>Decimal numbers

Decimal numbers are represented in Tacit in the same way that they are represented
...
Hexadecimal numbers are represented in Tacit using the uppercase letters `A` to `F`
...
Unlike decimal numbers, hexadecimal numbers are assumed to be positive in Tacit.

## <a name='printing'></a>Printing

### <a name='printing-numbers'></a>Printing numbers

Tacit provides commands for printing numbers in decimal and hexadecimal format.
...
Tacit allows the user to easily print literal text by using \` quotes.
...
## <a name='stack-maniplation-in-Tacit'></a>Stack Manipulation in Tacit

In Tacit, the stack is a central data structure that stores values temporarily.
...
Tacit uses numbers to define boolean values.
...
Tacit has a set of bitwise logical operators that can be used to manipulate bits. These operators are:
...
Here is an example of how to use the bitwise logical operators in Tacit:
...
Variables are named locations in memory that can store data. Tacit has a limited
number of global variables which have single letter names. In Tacit a variable can
be referred to by a singer letter from `a` to `z` so there are 26
global variables in Tacit. Global variables can be used to store numbers, strings, arrays, blocks, functions etc.
...
Tacit arrays are a type of data structure that can be used to store a collection of elements. Arrays are indexed, which means that each element in the array has a unique number associated with it. This number is called the index of the element.
In Tacit, array indexes start at 0

To create a Tacit array, you can use the following syntax:
...
In Tacit arrays can be nested inside one another.
...
Tacit by default declares arrays of 16 bit words however it is also possible to declare
and array of 8 bit byte values by using `\` which puts Tacit into `byte mode`.
...
Note: Tacit will leave byte mode (and return to normal word mode) after it executes a `]`, `?` or `!`

### <a name='memory-allocation'></a>Memory allocation

The final kind of memory allocation in Tacit is the simplest raw memory allocation on the heap.
...
Looping in Tacit is of the form
...
Tacit provides a special variable `/i` which acts as a loop counter. The counter counts up from zero. Just before the
...
Tacit's looping mechanism can also be used to execute code conditionally. In Tacit boolean `false` is represented
...
The syntax for a Tacit IF-THEN-ELSE or "if...else" operator in Tacit is and
...
Here is an example of a "if...else" operator in Tacit:
...
Here is another example of the "if...else" operator in Tacit. This time, instead of creating a string just to print it, the following
...
## <a name='functions-in-Tacit'></a>Functions in Tacit

You can put any code inside `:` and `;` block which tells Tacit to "execute this later".

Functions are stored in variables with uppercase letters. There are 26 variables
for storing functions in Tacit and use the uppercase letter A to Z.
...
Once you've assigned functions to variables, you can use them in your Tacit code.
...
Tacit code is not restricted to upper case variables. Functions an be declared without a
...
### <a name='using-Tacit-on-the-tec-1'></a>Using Tacit on the TEC-1

Tacit was designed for for small Z80 based systems but specifically with the small memory configuration
...
| /G       | execute Tacit code at address   | a -- ? |
